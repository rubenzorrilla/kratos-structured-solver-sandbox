...
def parse_scalar(file_path, dtype = float):
    data = []
    with open(file_path, 'r') as file:
        for line in file:
            data.append(dtype(line.strip()))
    return data
...
def parse_scalar_time(file_path, delimiter = " "):
    data = []
    with open(file_path, 'r') as file:
...
# Parse distance
distance = parse_scalar("distance.txt", dtype=float)
    ...
    KratosMultiphysics.Parameters("""
        {
            "result_file_configuration": {
                ...
                "plane_output": [],
                "nodal_results": [],
                "nodal_nonhistorical_results": ["VELOCITY","DISTANCE"],
                "nodal_flags_results": [],
                "gauss_point_results": [],
...
vtu_output = VtuOutputProcess(
    ...
    KratosMultiphysics.Parameters("""{
        ...
        "nodal_data_value_variables"        : ["VELOCITY","DISTANCE"],
        "nodal_flags"                       : [],
...
for step in step_vect:
    ...
    time_p, p_array = parse_scalar_time(f"p_{step}.txt")
    ...
    for elem, p_value in zip(output_model_part.Elements, list(p_array)):
        elem.SetValue(KratosMultiphysics.PRESSURE, p_value)
    # KratosMultiphysics.VariableUtils().SetValuesVector(output_model_part.Elements, KratosMultiphysics.PRESSURE, list(p_array)) //TODO: Do a variable utils method for elements
