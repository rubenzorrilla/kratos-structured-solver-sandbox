...
public:
              ...
              VectorType& rX)
    {
        ...
        VectorType r_k(mProblemSize, 0);
        VectorType aux(mProblemSize, 0);

        VectorType d_k(mProblemSize, 0);
        VectorType d_k_1(mProblemSize, 0);
        VectorType r_k_1(mProblemSize, 0);
        VectorType z_k(mProblemSize, 0);
        VectorType z_k_1(mProblemSize, 0);
        ...
        // GPU implementation of the gradient operator
        {
            ...
            auto d_k_gpu_buff   = sycl::buffer{d_k.data(),      cl::sycl::range<1>{mProblemSize}};
            auto d_k_1_gpu_buff = sycl::buffer{d_k_1.data(),    cl::sycl::range<1>{mProblemSize}};
            auto r_k_1_gpu_buff = sycl::buffer{r_k_1.data(),    cl::sycl::range<1>{mProblemSize}};
            auto z_k_gpu_buff   = sycl::buffer{z_k.data(),      cl::sycl::range<1>{mProblemSize}};
            auto z_k_1_gpu_buff = sycl::buffer{z_k_1.data(),    cl::sycl::range<1>{mProblemSize}};
            ...
            //     cgh.parallel_for<class GPU_PressureOperator_Apply>(cl::sycl::range<1>(mProblemSize), [=, this](cl::sycl::item<1> item) {
            ...
            queue.submit([&](cl::sycl::handler & cgh) {
                ...
                cgh.parallel_for(sycl::range<1>{static_cast<int>(mProblemSize)}, sycl::reduction(resultBuf, cgh, std::plus<double>()),
